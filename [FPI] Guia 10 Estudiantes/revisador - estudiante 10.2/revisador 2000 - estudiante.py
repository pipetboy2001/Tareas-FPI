...
def check_pw_validity(pw, max_length, min_length=8):
    lower_letters = "abcdefghijklmnopqrstuvwxyz"
    upper_letters = "ABCDEFGHIJKLMNOPQRSTUVWXYZ"
    numbers = "0123456789"
    symbols = "?!@#$%^&*"
    
    lowers = [c for c in pw if c in lower_letters]
    uppers = [c for c in pw if c in upper_letters]
    nums = [c for c in pw if c in numbers]
    symbs = [c for c in pw if c in symbols]
    ...
    if not (min_length <= len(pw) <= max_length):
        ...
        return 5
    elif len(lowers) == len(uppers) == len(nums) == len(symbs):
        return 6
    return 0
    ...
    if not formato:
        ...
        while i < len(testE):
            ...
            try:
                ...
                if len(testE[i]) == 2:
                    if testE[i][1] == '':
                        ...
                        entrada = ",".join(testE[i])
                    
                #print(entrada)
                with open ('./estudiantes/'+alumno,"r",encoding = "utf-8") as flujo:
                     data_estudiante_full = flujo.read()

                #####################################
                # Agregar llamado a función
                #####################################
                data_estudiante_full += "\n\n"
                data_estudiante_full += "print(gen_pw("+entrada+"))"
                #######################################
                         
                with open ('test.py',"w",encoding = "utf-8") as flujo2:
                     flujo2.write(data_estudiante_full)
            ...
            if error:
                ...
                if testE[i][1] == "":
                    resX = check_pw_validity(resultado,int(testE[i][0]))
                else:
                    resX = check_pw_validity(resultado,int(testE[i][0]),int(testE[i][1]))
                ...
                if resX ==0:
                    lista_linea.append("OK")
                else:
                    lista_linea.append("MAL")
